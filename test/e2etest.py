...
def test_twilio_phone_say_api():
    ...
    """
    signature = 'Fb6oNJJO1SdI35x2PFaSQC48eOY='
...
def test_twilio_phone_gather_api():
    ...
    re = requests.post(
        ...
        headers={'X-Twilio-Signature': 'DIYXm8OJAUCnpObo6bT3deuqdmQ='})
    assert re.status_code == 200
    assert re.content == (
...
def test_twilio_phone_gather_api_batch_message_id():
    ...
    re = requests.post(
        ...
        headers={'X-Twilio-Signature': '9CXm6vsBitJuotyDUwT9MruGBf8='})
    assert re.status_code == 200
    assert re.content == (
        '<?xml version="1.0" encoding="UTF-8"?><Response>'
        '<Pause length="2" />'
    ...
    re = requests.post(
        base_url + 'twilio/calls/gather', params=params,
        headers={'X-Twilio-Signature': '+PExWTIiqBcGBpHse0AgCM2iaFo='})
...
def test_twilio_phone_relay_api():
    ...
    re = requests.post(
        ...
        headers={'X-Twilio-Signature': '9SFAgHrRiVf3nUKWyLScFVLRDQQ='},
...
def test_twilio_message_relay_api():
    ...
    re = requests.post(
        ...
        params={'message_id': 1001},
        headers={'X-Twilio-Signature': 'HJ5K+JkhPP1THyWJe/m5d+M/CGE='},
